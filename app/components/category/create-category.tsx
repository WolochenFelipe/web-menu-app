...
const categorySchema = zfd.formData({
    name: z.string().nonempty("Nome é Obrigatório").min(3, "Mínimo de 3 Letras para o Nome"),
    description: z.string().nonempty("Descrição é Obrigatório").min(3, "Mínimo de 3 Letras para a Descrição"),
...
export default function CreateCategory({ tenantId }: { tenantId: string }) {
    const { register, handleSubmit, formState: { errors }, setValue, getValues } = useForm<CategorySchema>({
            ...
            trigger={
                <li>
                    <button className="w-full group flex items-center rounded-lg p-2 text-gray-900 hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">
                        ...
                        <span className="ms-3 ">Adicionar Categoria</span>
                    </button>
                </li>
            }
        >
                ...
                <form onSubmit={handleSubmit(handleSubmitMenu)} className="space-y-4">
                    <div>
                        ...
                        <div className='flex flex-col gap-1 mt-8'>
                            ...
                            />
                            {errors.description && <span className='mt-2 text-red-500'>{errors.description.message}</span>}
                        </div>
                        <div className='flex flex-col gap-1 mt-8'>
